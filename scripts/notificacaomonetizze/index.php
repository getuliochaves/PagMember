...
//INICIO Recebeu POST maior que 0(zero)
if(count($_POST) > 0){
	...
	//Valida o POST da Chave API
	$dados = $_POST;
	$chaveApi = $dados['chave_unica'];
	if($chaveApi == ''){
	$chaveApi = $_POST['token'];
	}

//INICIO Verificacao se o token recebido é igual ao token do banco
if($chaveApi == $token){

	//Inicio Validacao Se nao For Teste Local
  ...
  $nomeProduto = $dados['produto']['nome'];
	$idTransacao = $dados['venda']['codigo'];
	$idProd = $dados['produto']['codigo'];
	$nomeCli = $dados['comprador']['nome'];
	$emailCli = $dados['comprador']['email'];
	$valorProd = $dados['venda']['valor'];
	$nomeProd = $dados['produto']['nome'];
	$statusTrasacao = $dados['venda']['status'];

	//var_dump($statusTrasacao);

	if($idProd == '' or $idProd == null){
		$idProd    = $dados['assinatura']['codigo'];
	}

	if($statusTrasacao == '' or $idProd == null){
		$statusTrasacao    = $dados['assinatura']['status'];
	}


	switch($statusTrasacao){
	case 'Aguardando pagamento': $statusCompra = 'Aguardando pagamento';
		$statusTrasacaoOk = 1;
		break;


		case 'Completa': $statusCompra = 'Aprovado';
			$statusTrasacaoOk = 3;
		break;

		case 'Ativa': $statusCompra = 'Aprovado';
			$statusTrasacaoOk = 3;
		break;

		case 'Finalizada': $statusCompra = 'Aprovado';
			$statusTrasacaoOk = 3;
		break;

		case 'Cancelada': $statusCompra = 'Cancelada';
			$statusTrasacaoOk = 6;
		break;

		case 'Inadimplente': $statusCompra = 'Cancelada';
			$statusTrasacaoOk = 6;
		break;

		case 'Bloqueada': $statusCompra = 'Cancelada';
			$statusTrasacaoOk = 6;
		break;

		case 'Devolvida': $statusCompra = 'Cancelada';
			$statusTrasacaoOk = 6;
		break;
...
};//FIM Validacao SE NAO FOR Teste Local
...
//Grava Dados do Usuario Executados No POST
	...
	//Fim Recebeu POST maior que 0(zero)
}else{
